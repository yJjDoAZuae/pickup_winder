...
{
    char Str[ARRAY_LEN];
    snprintf(Str, sizeof(Str), "%.*lf", Digit + 1, Number);

    uint16_t max_len = strlen(Str);
    if (max_len < 2 || max_len > ARRAY_LEN) {
        return;
    }

    // char *pStr = (char *)malloc(max_len * sizeof(char));
    char *pStr = (char *)malloc(ARRAY_LEN * sizeof(char));
    if (pStr == NULL) {
        return;
    }

    // memcpy(pStr, Str, (max_len - 1));
    // *(pStr + max_len - 1) = '\0';
    // if (Digit == 0)
    //     *(pStr + max_len - 2) = '\0';
    // // show
